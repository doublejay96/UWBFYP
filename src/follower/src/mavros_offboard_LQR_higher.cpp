...
//This LQR_higher operates on 6 state variables (x, y, z, vx, vy, vz) and 3 input variables (ux, uy, uz)
...
float LQR_Q_x = 1, LQR_Q_y = 1, LQR_Q_z = 1, LQR_Q_vx = 1, LQR_Q_vy = 1, LQR_Q_vz = 1;//Diagonal elements of the Q matrix
...
Eigen::Vector3f LQR(Eigen::Matrix<float,6,1> state_error, Eigen::Matrix<float,6,6> Q, Eigen::Matrix<float,3,3> R, Eigen::Matrix<float,6,6> A, Eigen::Matrix<float,6,3> B) {
    int N = 50;//time-horizon
    std::vector<Eigen::Matrix<float,6,6>> P_vec (N+1);//Positive,definite,symmetric solution to DARE
    P_vec[N] = Q;//terminal condition
    //Pre-compute transposes of A, B to save function calls, A,B do not vary
	//Actually not needed if A,B square and symmetric but just in case
    Eigen::Matrix<float,6,6> A_t = A.transpose();
    Eigen::Matrix<float,3,6> B_t = B.transpose();
    ...
    std::vector<Eigen::Matrix<float,3,6>> K_vec (N);//feedback gain matrix K
...
int main(int argc, char** argv) {
	ros::init(argc, argv, "mavros_offboard_LQR_higher");//initialise the node, name it "mavros_offboard_LQR"  
	ros::NodeHandle nh; //construct the first NodeHandle to fully initialise, handle contains communication fns
	int use_LQR_higher_controller = 0;
    nh.param("use_LQR_higher_controller", use_LQR_higher_controller, 0);//if not set, default to 0 (PID)
    if (use_LQR_higher_controller == 0) {
        ROS_INFO("Not using LQR_higher controller");
        return 0;//exit the node immediately
    } else if (use_LQR_higher_controller == 1) {
        ROS_INFO("Using LQR_higher controller");
    } else if (use_LQR_higher_controller == 2) {
		ROS_INFO("Using LQR_higher controller in dummy mode");
	}
	...
	Eigen::Matrix<float,6,6> A;//Define A,B,Q,R,state,control EXPLICITLY
	Eigen::Matrix<float,6,3> B;
	Eigen::Matrix<float,6,6> Q;
	Eigen::Matrix<float,3,3> R;
	Eigen::Matrix<float,6,1> state;
	Eigen::Matrix<float,3,1> control;
	//Initialise the A, B matrices from state space model file
    std::string SSmodel_path = "state_space_model_higher.csv";
	...
	SSmodel_file.close();
	A = Eigen::Map<Eigen::Matrix<float,6,6>>(SSmodel_A.data());
    B = Eigen::Map<Eigen::Matrix<float,6,3>>(SSmodel_B.data());
    //Initialise the Q, R matrices
	nh.getParam("LQR_Q_x", LQR_Q_x);
	nh.getParam("LQR_Q_y", LQR_Q_y);
	nh.getParam("LQR_Q_z", LQR_Q_z);
	nh.getParam("LQR_Q_vx", LQR_Q_vx);
	nh.getParam("LQR_Q_vy", LQR_Q_vy);
	nh.getParam("LQR_Q_vz", LQR_Q_vz);
	nh.getParam("LQR_R_vx", LQR_R_vx);
	nh.getParam("LQR_R_vy", LQR_R_vy);
	nh.getParam("LQR_R_vz", LQR_R_vz);
	Q = Eigen::Matrix<float,6,6>::Zero();//diagonal matrix, only non-zero on diagonal elements
	Q(0,0) = LQR_Q_x;
	Q(1,1) = LQR_Q_y;
	Q(2,2) = LQR_Q_z;
	Q(3,3) = LQR_Q_vx;
	...
	while (ros::ok() && current_state.connected) {//begin LQR controller
		...
		state << x_error, y_error, z_error, derivative_x, derivative_y, derivative_z;//update the current state with the UWB readings
        ...
        if (flightStage == 2) {
            if (use_LQR_higher_controller == 1) {
