...
int main(int argc, char** argv) {
	...
	double follower_altitude;//Altitude of the follower in simulation (in m)
	...
	ros::Publisher altitude_pub = nh.advertise<follower::sim_altitude>("follower1/sim_altitude", 1000);
	...
	message.P = 0;
	gazebo_msgs::GetModelState follower_altitude_state;
	follower_altitude_state.request.model_name = "iris1";//get the state of iris1, the follower
	follower::sim_altitude sim_altitude_message;//create the altitude message (for sims)
	while (ros::ok()) {
		if (model_state_client.call(leader_relative_state)) {
			Xcm_sim = leader_relative_state.response.pose.position.y * 100;//get relative position (convert to cm)
			...
			ROS_INFO("uwb_simulator: could not get relative leader position");
		}
		if (model_state_client.call(follower_altitude_state)) {
			follower_altitude = follower_altitude_state.response.pose.position.z;
			sim_altitude_message.altitude = follower_altitude;
			altitude_pub.publish(sim_altitude_message);
		} else {
			ROS_INFO("uwb_simulator: could not get follower altitude");
		}
